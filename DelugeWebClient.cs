...
namespace TorrentBackend
{
    public class DelugeWebClient : WebClient, IDisposable
    {
        ...
        public async Task<List<TorrentObject>> GetCurrentTorrents()
        {
            ...
            List<TorrentObject> values = new List<TorrentObject>();
            foreach(Newtonsoft.Json.Linq.JToken tok in json.SelectTokens("result.torrents.*"))
            {
                ...
                values.Add(new TorrentObject() {Hash = hash, Name = name, Progress = progress});
            }
            return values;
        }
